...
#include <stdlib.h>
#include <string.h>
int re_match(const char *pat, const char *text, unsigned flags, int *caps, int ncaps, void *, unsigned);
int base_re_match(const char *pat, const char *text, unsigned flags, int *caps, int ncaps, void *, unsigned);
...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < total; i++) {
        ...
        na = re_match(pat, txt, fl, ca, nc, workspace, sizeof workspace);
        nb = base_re_match(pat, txt, fl, cb, nc, workspace, sizeof workspace);
        if (na != nb || (na == 1 && memcmp(ca, cb, sizeof ca))) {
            if (++diffs <= 10)
