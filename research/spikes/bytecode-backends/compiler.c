...
#if ENABLE_CHAR || ENABLE_RANGE
    if (!negated && count == 1 && compiler->scratch->ranges[0].lower > 0 &&
        compiler->scratch->ranges[0].upper < RE_ASCII_LIMIT) {
#if ENABLE_CHAR
        if (compiler->scratch->ranges[0].lower == compiler->scratch->ranges[0].upper) {
            byte(compiler, RE_OP_CHAR);
            byte(compiler, compiler->scratch->ranges[0].lower);
            return;
        }
#endif
#if ENABLE_RANGE
        byte(compiler, RE_OP_ASCII_RANGE);
        byte(compiler, compiler->scratch->ranges[0].lower);
        byte(compiler, compiler->scratch->ranges[0].upper);
        return;
#endif
    }
#endif
